...
export function OrcamentoStatusAcoes({ id }: { id: number }) {
  const router = useRouter();
  const [carregando, setCarregando] = useState<string | null>(null);

  async function mudarStatus(status: "ACEITO" | "RECUSADO") {
    setCarregando(status);
    try {
      await apiFetch(`/api/orcamentos/${id}`, {
        method: "PATCH",
